...
func (c *Canvas) SetFont(name string, size float64) {
	c.gc.SetFontData(draw2d.FontData{
		Name: name,
	})
	c.gc.SetFontSize(size)
}
func (c *Canvas) FillText(x, y float64, text string) {
	c.gc.FillStringAt(text, x, y)
}
